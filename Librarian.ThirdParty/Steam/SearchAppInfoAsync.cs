    ...
    public partial class SteamApiService
    {
        public async Task<List<AppInfo>> SearchAppInfoAsync(string nameLike, CancellationToken ct = default)
        {
            try
            {
                ...
                {
                    _logger.LogError("Failed to search Steam apps: {StatusCode} {ErrorMessage}", response.StatusCode, response.ErrorMessage);
                ...
                foreach (var item in searchResults.Take(10))
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        _logger.LogError(ex, "Error retrieving details for Steam app {AppId}: {ErrorMessage}", item.AppId, ex.Message);
                    }
                }
