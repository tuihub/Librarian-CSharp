...
public partial class BangumiApiService
{
    public async Task<List<AppInfo>> SearchAppInfoAsync(string nameLike, CancellationToken ct = default)
    {
        ...
        var searchResultsArray = JArray.Parse(response.Content);
        ...
        foreach (var result in searchResultsArray.Take(10))
            try
            {
                var appId = result["id"]?.ToString() ?? "";
            ...
            catch (Exception ex)
            {
                ...
                var subjectId = result["id"]?.ToString() ?? "unknown";
