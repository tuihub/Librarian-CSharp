    ...
    public Task<AppInfo> ParseRawAppInfoAsync(string rawDataJson, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var shortDescription = ((string?)respObj.summary?.ToString())?.Length > 97
                ? ((string?)respObj.summary?.ToString())?[..97] + "..."
                : respObj.summary?.ToString();
            ...
            if (infobox != null)
                foreach (var parsedObject in infobox.Children<JObject>())
            ...
            foreach (var info in respObj.infobox)
                if (info.key == ALIAS_KEY)
                    foreach (var altName in info.value)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(name) && !altNames.Contains(name)) altNames.Add(name);
                    }

            _logger.LogDebug("Extracted {AltNameCount} alternative names for ID: {AppId}", altNames.Count,
                appIdFromRawInfo);
            ...
            return Task.FromResult(new AppInfo
            {
                ...
                Name = (string.IsNullOrWhiteSpace(respObj.name_cn.ToString())
                    ? respObj.name.ToString()
                    : respObj.name_cn.ToString()) ?? string.Empty,
                Type = AppType.Game,
        ...
        {
            _logger.LogError(ex, "Failed to parse Bangumi data for ID: {AppId}. Error: {ErrorMessage}",
                appIdFromRawInfo, ex.Message);
