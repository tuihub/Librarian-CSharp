...
public partial class BangumiApiService
{
    public async Task<AppInfo> GetAppInfoAsync(string appIdStr, CancellationToken ct = default)
    {
        ...
        {
            _logger.LogError("Bangumi API returned non-success status code: {StatusCode} for ID: {AppId}",
                response.StatusCode, appId);
            throw new Exception("Bangumi API returned non-success status code: " + response.StatusCode);
        }
