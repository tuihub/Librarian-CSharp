...
public partial class VndbTcpApiService
{
    public async Task<List<AppInfo>> SearchAppInfoAsync(string nameLike, CancellationToken ct = default)
    {
        ...
        var searchResults =
            await client.GetVisualNovelAsync(VndbFilters.Search.Fuzzy(nameLike), VndbFlags.FullVisualNovel);
        ...
        foreach (var vn in searchResults.Take(10))
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Error processing VNDB result for visual novel {VnId}: {ErrorMessage}", vn.Id,
                    ex.Message);
            }
