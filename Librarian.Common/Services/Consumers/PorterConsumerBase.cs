    ...
    where TMessage : PorterMessageBase
{
    ...
    public async Task Consume(ConsumeContext<TMessage> context)
    {
        ...
        try
        {
            ...
            _logger.LogDebug("Successfully processed message, feature: {Feature}, region: {Region}", _featureName,
                region);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex,
                "Consumer error occurred while processing message, feature: {Feature}, region: {Region}", _featureName,
                region);
            throw;
        }
    }
