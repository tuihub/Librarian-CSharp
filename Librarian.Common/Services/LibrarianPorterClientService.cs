...
public class LibrarianPorterClientService
{
    ...
    {
        return InvokeWithPorterInnerAsync(
            "GetAccount",
        ...
        CancellationToken cancellationToken = default)
    {
        return InvokeWithPorterInnerAsync(
            ...
            $"{PorterFeature.AppInfoSource}.{featureRequest.Id}",
            string.IsNullOrWhiteSpace(featureRequest.Region) ? null : featureRequest.Region,
            async (client, token) =>
            {
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (porter == null)
            {
                _logger.LogWarning("No Porter instance available for feature: {Feature} in region: {Region}",
                    featureName, region);
                return default;
            }
        ...
        {
            _logger.LogError(ex, "Resource exhausted while calling Porter. feature: {Feature}, region: {Region}",
                featureName, region);
            if (porter != null)
        ...
        {
            _logger.LogError(ex, "Error while calling Porter. feature: {Feature}, region: {Region}", featureName,
                region);
            if (porter != null)
                await _porterManagementService.DecreasePorterInstanceFeatureConcurrencyAsync(porter.Id, featureName);
            throw;
        ...
        finally
        {
            ...
            if (porter != null)
            {
                ...
                _logger.LogDebug("Released Porter instance: {PorterId} for feature: {Feature}, region: {Region}",
                    porter.Id, featureName, region);
            }
        }
    }
}