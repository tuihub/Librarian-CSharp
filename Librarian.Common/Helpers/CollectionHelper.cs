namespace Librarian.Common.Helpers
{
    public static class CollectionHelper
    {
        ...
        /// <param name="contentComparer">Function to compare old and converted new items. If null, default equality comparer will be used.</param>
        /// <returns>Tuple containing collections of items to remove, add, and update</returns>
        public static (ICollection<TOld> ToRemove, ICollection<TOld> ToAdd, ICollection<(TOld OldItem, TNew NewItem)> ToUpdate)
            ...
            where TKey : notnull
        {
            ...
            foreach (var newItem in newCollection)
            {
                ...
                if (oldDict.TryGetValue(key, out var oldItem))
                {
                    ...
                    if (!contentComparer(oldItem, convertedNewItem))
                    {
                        toUpdate.Add((oldItem, newItem));
                    }
                }
