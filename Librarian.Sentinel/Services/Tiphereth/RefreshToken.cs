...
namespace Librarian.Sentinel.Services
{
    public partial class SephirahSentinelService
    {
        ...
        public override async Task<RefreshTokenResponse> RefreshToken(RefreshTokenRequest request, ServerCallContext context)
        {
            ...
            if (DateTime.UtcNow + TimeSpan.FromMinutes(Common.GlobalContext.JwtConfig.SentinelRefreshTokenExpireMinutes * 0.25) > expireTime)
            {
                refreshToken = JwtUtil.GenerateSentinelRefreshToken(internalId);
            }
            // return new tokens
