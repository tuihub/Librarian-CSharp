...
public partial class SephirahSentinelService
{
    ...
    public override async Task<ReportAppBinariesResponse> ReportAppBinaries(ReportAppBinariesRequest request,
        ServerCallContext context)
    {
        ...
        var commitSnapshot = request.HasCommitSnapshot && request.CommitSnapshot;
        ...
        foreach (var library in sentinel.SentinelLibraries)
        {
            ...
            var (toRemove, toAdd, toUpdate) = CollectionHelper
                .CompareCollections<DbModels.SentinelAppBinary, SentinelLibraryAppBinary, string>(
                    existingAppBinaries,
                    reportedBinaries,
        ...
        if (commitSnapshot) response.CommitSnapshotSuccess = true;
