...
{
    public override async Task<SearchAppInfoResponse> SearchAppInfo(SearchAppInfoRequest request,
        ServerCallContext context)
    {
        ...
        {
            var errorMsg =
                $"SearchAppInfo config validation failed: {string.Join("; ", validationErrors.Select(e => e.ToString()))}";
