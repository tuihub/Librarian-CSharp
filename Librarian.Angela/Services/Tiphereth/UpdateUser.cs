...
namespace Librarian.Angela.Services
{
    public partial class AngelaService
    {
        ...
        public override async Task<UpdateUserResponse> UpdateUser(UpdateUserRequest request, ServerCallContext context)
        {
            ...
            if (request.User.Type != UserType.Unspecified)
            {
                user.Type = ConvertToDbUserType(request.User.Type);
            }

            if (request.User.Status != UserStatus.Unspecified)
            {
                user.Status = ConvertToDbUserStatus(request.User.Status);
            }
