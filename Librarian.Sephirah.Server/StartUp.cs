...
using Librarian.Angela.Authorization;
...
public static class StartUp
{
    public static void ConfigureServices(WebApplicationBuilder builder,
        (ApplicationDbType dbType, string dbConnStr)? testDb)
    {
        ...
        builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
            ...
            // RefreshToken Auth
            .AddJwtBearer("RefreshToken",
                options => options.GetJwtBearerOptions(GlobalContext.JwtConfig.RefreshTokenAudience))
            // UploadToken Auth
            .AddJwtBearer("UploadToken",
                options => options.GetJwtBearerOptions(GlobalContext.JwtConfig.UploadTokenAudience))
            // DownloadToken Auth
        ...
        builder.Services.AddAuthorizationBuilder()
            .AddPolicy("AngelaAccess", policy =>
                policy.Requirements.Add(new AngelaAuthorizationRequirement()));
        builder.Services.AddHttpContextAccessor();
        builder.Services
            .AddSingleton<IAuthorizationHandler, AngelaAuthorizationHandler>();
        ...
        else
        {
            throw new NotSupportedException(
                $"Unsupported transport type: {GlobalContext.MassTransitConfig.TransportType}");
        }
    }
