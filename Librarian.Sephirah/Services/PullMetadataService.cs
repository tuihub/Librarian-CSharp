...
public class PullMetadataService
{
    ...
    private void InitializeStaticPorterInstances()
    {
        ...
        _logger.LogInformation("Initializing {Count} static Porter instances",
            _sephirahContext.StaticPorterInstances.Count);
        ...
        foreach (var porter in _sephirahContext.StaticPorterInstances)
        {
            ...
            foreach (var tag in porter.Tags)
            {
                ...
                try
                {
                    ...
                    _logger.LogInformation("Successfully created endpoint for static Porter ({Id}, {Tag})", porter.Id,
                        tag);
                }
                catch (Exception ex)
                {
    ...
    private async void MainPullMetadataThread(CancellationToken ct)
    {
        ...
        {
            _logger.LogWarning("Consul client is not configured.");
            return;
        }
        ...
        while (true)
        {
            ...
            {
                // stop all endpoints
                foreach (var endpoint in _platformEndpoints.Values) await endpoint.StopAsync();
                _platformEndpoints.Clear();
                break;
            }

            _logger.LogInformation(
                $"Got {serviceName}, WaitIndex = {consulQueryOptions.WaitIndex}, LastIndex = {response.LastIndex}");
            if (response.LastIndex != consulQueryOptions.WaitIndex)
            {
                ...
                {
                    _logger.LogInformation(
                        $"Stopping endpoint for ({endpointToRemove.Item1}, {endpointToRemove.Item2})");
