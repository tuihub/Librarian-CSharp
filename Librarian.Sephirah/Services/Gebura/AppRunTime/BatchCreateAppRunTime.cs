...
namespace Librarian.Sephirah.Services
{
    ...
    {
        [Authorize]
        public override Task<BatchCreateAppRunTimeResponse> BatchCreateAppRunTime(BatchCreateAppRunTimeRequest request, ServerCallContext context)
        {
            ...
            foreach (var appRunTimeReq in request.AppRunTimes)
            {
                ...
                // Check Device
                var device = _dbContext.Devices.Include(x => x.Users).SingleOrDefault(x => x.Id == appRunTimeReq.DeviceId.Id);
                if (device == null)
                {
                ...
                app.AppRunTimes.Add(new DbAppRunTime
                {
                    ...
                    EndDateTime = appRunTimeReq.RunTime.StartTime.ToDateTime() + appRunTimeReq.RunTime.Duration.ToTimeSpan(),
