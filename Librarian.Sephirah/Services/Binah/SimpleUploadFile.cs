...
public partial class SephirahService : LibrarianSephirahService.LibrarianSephirahServiceBase
{
    ...
    public override async Task SimpleUploadFile(IAsyncStreamReader<SimpleUploadFileRequest> requestStream,
        IServerStreamWriter<SimpleUploadFileResponse> responseStream, ServerCallContext context)
    {
        ...
        try
        {
            ...
            var pipeStreamClient4Sha256 =
                new AnonymousPipeClientStream(pipeStreamServer4Sha256.GetClientHandleAsString());
            var pipeStreamServer4Minio = new AnonymousPipeServerStream();
            var pipeStreamClient4Minio =
                new AnonymousPipeClientStream(pipeStreamServer4Minio.GetClientHandleAsString());
            ...
            var writeTask = Task.Run(async () =>
            {
                ...
                {
                    if (writeTaskCancellationTokenSource.IsCancellationRequested) break;
                    Thread.Sleep(200);
                    if (executionNum % 15 == 0)
                        await responseStream.WriteAsync(new SimpleUploadFileResponse
            ...
            if (!sha256Task.Result.SequenceEqual(fileMetadata.Sha256.ToArray()))
            {
        ...
        catch (Exception ex)
        {
            ...
            throw new RpcException(new Status(StatusCode.Internal, $"Error, inner exception: {ex.Message}"));
        }
    }
}